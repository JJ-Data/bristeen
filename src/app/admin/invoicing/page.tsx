...
import { fmt } from '@/lib/invoice-format'
...
export default async function InvoicingDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-800">Invoicing</h2>
        <p className="text-gray-500 text-sm mt-1">Create and manage invoices and receipts.</p>
      </div>

      {/* Main Actions */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Link href="/admin/invoicing/new?type=invoice"
          className="group bg-white rounded-2xl p-6 shadow-sm border-2 border-transparent hover:border-amber-400 transition flex items-center gap-4">
        ...
        <Link href="/admin/invoicing/new?type=receipt"
          className="group bg-white rounded-2xl p-6 shadow-sm border-2 border-transparent hover:border-green-400 transition flex items-center gap-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm text-center col-span-2 sm:col-span-1">
          <Link href="/admin/invoicing/items" className="flex flex-col items-center group">
      ...
      {recentDocs && recentDocs.length > 0 && (
        <div className="bg-white rounded-2xl shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/admin/invoicing/history" className="text-sm text-amber-500 hover:underline">View all</Link>
          </div>
          <div className="space-y-2">
            {recentDocs.map(doc => (
              <Link key={doc.id} href={`/admin/invoicing/history/${doc.id}`}
                className="flex items-center justify-between p-3 rounded-xl hover:bg-amber-50 transition">
                <div>
