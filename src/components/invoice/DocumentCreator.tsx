...
import { Item, InvoiceItem, DocumentType } from '@/types/invoice'
...
export default function DocumentCreator({ items, existingClients }: Props) {
  ...
  const [clientSuggestions, setClientSuggestions] = useState<string[]>(existingClients)
  ...
  function handleClientInput(value: string) {
    setClientName(value)
    if (value.length > 0) {
      const filtered = clientSuggestions.filter(c => c.toLowerCase().includes(value.toLowerCase()))
  ...
  async function handleSave() {
    ...
    setSaving(false)
    router.push(`/admin/invoicing/history/${invoice.id}`)
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm space-y-3">
        ...
        <div ref={clientRef} className="relative">
          <input
            ...
            onFocus={() => clientName && setShowSuggestions(clientSuggestions.some(c => c.toLowerCase().includes(clientName.toLowerCase())))}
          ...
          {showSuggestions && (
            <ul className="absolute z-10 w-full bg-white border border-gray-200 rounded-lg shadow-lg mt-1 max-h-40 overflow-y-auto">
              {clientSuggestions.filter(c => c.toLowerCase().includes(clientName.toLowerCase())).map(name => (
                <li key={name} onClick={() => { setClientName(name); setShowSuggestions(false) }}
          ...
          <div className="space-y-2 mt-2">
            ...
            {lineItems.map((line, i) => (
              <div key={i} className="grid grid-cols-12 gap-2 items-center">
                ...
                />
                <span className="col-span-1 text-sm text-right text-gray-700 font-medium">{fmt(line.total)}</span>
            ...
            <div className="pt-2 border-t border-gray-100 space-y-2">
              ...
              <div className="flex items-center justify-between text-sm text-gray-500">
                <div className="flex items-center gap-2">
                  <span>VAT / Tax</span>
                  <input type="number" min="0" max="100" step="0.5" value={vatRate}
      ...
      <button onClick={handleSave} disabled={saving || !clientName || lineItems.length === 0}
