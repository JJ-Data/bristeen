...
import { Invoice, InvoiceItem, Item } from '@/types/invoice'
...
export default function InvoiceEditor({ invoice, onCancel, onSaved }: Props) {
  ...
  useEffect(() => {
    ...
    supabase.from('clients').select('name').then(({ data }) => {
      if (data) setClientSuggestions(data.map((c: { name: string }) => c.name))
    })
  }, [])
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h3 className="font-bold text-gray-800 text-lg">
          Edit {invoice.type.charAt(0).toUpperCase() + invoice.type.slice(1)}
        </h3>
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm space-y-3">
        ...
        <div ref={clientRef} className="relative">
          <input
            ...
            onFocus={() => setShowSuggestions(clientSuggestions.some(c => c.toLowerCase().includes(clientName.toLowerCase())))}
